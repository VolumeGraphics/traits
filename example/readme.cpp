...
namespace trait_examples
{
    ...
    {
        Method<"bar", void(char const& lvalue)>,
        Method<"bar", void(char& lvalue)>,
        Method<"bar", void(char&& lvalue)>,
        ...
        protected:
            bool checkGeoConstraints(Vector3i const& point)
            {
                for (const auto& constraint : mGeoConstraints)
                    if (not constraint.checkPoint(point))
                        return false;
    ...
    inline namespace with_traits
    {
        ...
        constexpr auto GeoConstraint = trait
        {
            Method<"setBreaking", void (bool breaking)>,
            Method<"isBreaking" , bool () const>,
        ...
        protected:
            bool checkGeoConstraints(Vector3i const& point)
            {
                return std::ranges::all_of (mGeoConstraints, [&] (auto const& constraint) { return constraint.check (point); });
            }

        private:
            // NOTE: we could also make SomeGeoConstraint a public type definition and accept SomeGeoConstraint arguments in our API
            using SomeGeoConstraint = some<GeoConstraint, buffer<112>, inlined { Method<"check", bool(const Vector3i& point) const> }>;
